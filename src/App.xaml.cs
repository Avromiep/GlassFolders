...
public partial class App : Application
{
    ...
    public const string AppVersion = "0.3.46";
    ...
    protected override void OnStartup(StartupEventArgs e)
    {
        ...
        if (e.Args.Length >= 1 && e.Args[0].Equals("--shotmgr2", StringComparison.OrdinalIgnoreCase))
        {
            try
            {
                _store = new FolderStore();
                var win = new ManagerWindow(_store)
                { WindowStartupLocation = WindowStartupLocation.Manual, Left = -32000, Top = 60 };
                win.Show();
                var outPng = e.Args.Length >= 2 ? e.Args[1] : "manager.png";
        ...
        Task.Run(() =>
        {
            try
            {
                foreach (var f in _store.ListFolders())
                    ImageHelper.Prewarm(f.FirstPagePaths(), 128);
            }
            catch { }
        });
    }
